...
const REALISTIC_PITCH_TEMPLATES = [
  {
    problem: 'Software teams spend 30% of sprint time writing documentation and manual API tests.',
    solution: 'AI agent that automatically writes integration tests and maintains documentation directly from pull requests.',
    whoPays: 'B2B SaaS engineering teams ($99/developer/month).',
    whyNow: 'LLM context windows and tool-calling capabilities reached production readiness in 2025.',
  ...
  {
    problem: 'E-commerce brands lose 15% of revenue to return fraud and bracket shopping.',
    solution: 'Predictive return risk scoring API for Shopify checkout that flags suspicious accounts before shipping.',
    whoPays: 'DTC e-commerce merchants doing >$1M GMV.',
    whyNow: 'Online return rates surged to 24% in 2025, costing brands billions.',
  ...
  {
    problem: 'Commercial buildings waste up to 40% of energy on HVAC due to static schedules.',
    solution: 'IoT sensor network + ML controller that dynamically adjusts heating and cooling based on real-time occupancy.',
    whoPays: 'Commercial property managers and office building owners.',
    whyNow: 'New EU and US carbon compliance laws impose steep penalties starting 2026.',
    ask: '$750,000 seed round.',
    category: 'Climate',
    demoClipUrl: '',
    tractionSnapshot: '2 office towers piloted, 34% documented energy reduction.',
  },
  {
    problem: 'Creators struggle to monetize short-form video audiences without sponsorships.',
    solution: 'Micro-tipping platform where fans unlock exclusive 15-second bonus clips for $0.50.',
    whoPays: 'Gen-Z consumers & TikTok creators taking a 10% platform fee split.',
    whyNow: 'Micropayments are finally viable with instant digital wallets.',
    ask: '$250,000 pre-seed.',
    category: 'Consumer',
    demoClipUrl: '',
    tractionSnapshot: '15,000 registered fans, $12k processed volume in month 1.',
  },
  {
    problem: 'Healthcare clinics waste hours managing patient follow-ups and prescription renewals.',
    solution: 'Voice-AI assistant that calls patients post-op, logs symptoms, and syncs directly with EHR systems.',
    ...
    category: 'Health',
    demoClipUrl: '',
    tractionSnapshot: '8 clinic contracts signed, 1,200 automated calls completed.',
  },
  {
    problem: 'Solana and Ethereum cross-chain swaps take multiple steps and high slippage.',
    solution: 'Zero-knowledge cross-chain liquidity protocol for instant 1-click token swaps with zero bridge exposure.',
  ...
  },
  {
    ...
    solution: 'Gamified virtual HQ where new hires solve onboarding quests and interact with teammates in 3D.',
    whoPays: 'HR leaders at remote-first companies with >50 employees.',
    whyNow: 'Remote work stabilized as permanent standard, but 90-day retention fell by 18%.',
  ...
  {
    problem: 'High school students lack personalized tutoring for SAT/ACT prep.',
    solution: 'AI tutor that adapts question difficulty in real time and predicts test score improvements with 95% accuracy.',
    whoPays: 'Parents via $29/month subscription.',
    whyNow: 'Adaptive learning models outperform traditional test prep courses at 1/10th the price.',
    ask: '$350,000 pre-seed.',
    category: 'Education',
    demoClipUrl: '',
    tractionSnapshot: '2,400 active students, average 140-point SAT score increase.',
  },
];
...
const getRandomUniqueComment = (category?: string, existingTexts: Set<string> = new Set(), username?: string): string => {
  const categoryPool = category && REALISTIC_COMMENTS_BY_CATEGORY[category] ? REALISTIC_COMMENTS_BY_CATEGORY[category] : [];
...
const ALBANIAN_FOUNDER_USERNAMES = [
  'ermal_k', 'arben_tech', 'gentian_dev', 'valon_b', 'besart_ventures',
  'drilon_m', 'agron_builds', 'liridon_labs', 'taulant_ai', 'teuta_ventures',
  'rozafa_tech', 'doruntina_dev', 'era_digital', 'bora_labs', 'zana_vc',
];

const MACEDONIAN_FOUNDER_USERNAMES = [
  'marko_tech', 'stefan_labs', 'trajche_dev', 'nikola_ai', 'bojan_builds',
  'goran_ventures', 'filip_m', 'blagoj_saas', 'elena_innovates', 'ivana_studio',
  'angela_tech', 'viktorija_ai',
];

const WORLDWIDE_FOUNDER_USERNAMES = [
  'marcus_dev', 'sarah_ai', 'chen_labs', 'hiroshi_tech', 'lucas_builder',
  'mateo_saas', 'emma_innovates', 'julian_vc', 'maya_labs', 'david_code',
  'noah_ventures', 'sophia_tech', 'alex_found', 'oliver_builds',
];

const ALBANIAN_INVESTOR_USERNAMES = [
  'ylli_cap', 'ilir_invests', 'gent_vc', 'blerim_angel', 'agim_capital',
  'artina_funds', 'altin_growth', 'dardan_holdings', 'vlora_vc', 'shpend_invest',
];

const MACEDONIAN_INVESTOR_USERNAMES = [
  'aleksandar_vc', 'petar_capital', 'dejan_funds', 'simona_angel', 'darko_invest',
  'milan_equity', 'kristijan_cap', 'zoran_angel',
];

const WORLDWIDE_INVESTOR_USERNAMES = [
  'oliver_cap', 'chloe_vc', 'liam_invests', 'isabella_fund', 'leo_growth',
  'ethan_angel', 'aria_holdings', 'jack_venture', 'noah_capital', 'hannah_vc',
];

const getRandomFounderUsername = () => {
  const pool = [...ALBANIAN_FOUNDER_USERNAMES, ...MACEDONIAN_FOUNDER_USERNAMES, ...WORLDWIDE_FOUNDER_USERNAMES];
...
const getRandomInvestorUsername = () => {
  const pool = [...ALBANIAN_INVESTOR_USERNAMES, ...MACEDONIAN_INVESTOR_USERNAMES, ...WORLDWIDE_INVESTOR_USERNAMES];
  const base = faker.helpers.arrayElement(pool);
  const suffix = Math.floor(1000 + Math.random() * 9000);
  return `${base}_${suffix}`;
};
...
export class SimulatorService implements OnApplicationBootstrap {
      ...
      for (const startup of startups) {
        ...
        for (const comment of startup.comments) {
          ...
          if (seenTexts.has(comment.text) || isGenericLegacy) {
            const newText = getRandomUniqueComment(startup.category, seenTexts, comment.user?.username);
            comment.text = newText;
            await this.commentRepo.save(comment);
            this.logger.log(`Enriched comment ID ${comment.id} for startup "${startup.problem.slice(0, 20)}..." -> "${newText}"`);
          } else {
  ...
  private async cleanupLegacyUsernames() {
    try {
      ...
      for (const u of legacyUsers) {
        if (u.username.startsWith('founder_') || u.username.startsWith('investor_')) {
          const isFounder = u.role === 'founder' || u.username.startsWith('founder_');
          const newUsername = isFounder ? getRandomFounderUsername() : getRandomInvestorUsername();
          u.username = newUsername;
          try {
            await this.userRepo.save(u);
            this.logger.log(`Cleaned up legacy username -> updated to @${u.username}`);
          } catch (e) {
  ...
  private async bootstrapInitialDataIfNeeded() {
    ...
    if (startupCount >= 4 && userCount >= 6) {
      this.logger.log(`Database already has ${startupCount} startups and ${userCount} users. Simulator running in background mode.`);
      return;
    }

    this.logger.log('Seeding initial simulated ecosystem (founders, pitches, investors, and investment histories)...');
    ...
    const founderUsernamesPool = [
      'ermal_k_24', 'marko_tech_91', 'arben_dev_33', 'sarah_ai_88', 'besart_ventures_12',
    ];
    ...
    const investorUsernamesPool = [
      'ylli_cap_45', 'aleksandar_vc_77', 'liam_invests_19', 'gent_vc_82', 'petar_capital_34',
      'oliver_cap_66', 'drilon_invest_53', 'simona_angel_90',
    ];
    ...
    for (const startup of createdStartups) {
      ...
      for (let j = 0; j < numInvestments; j++) {
        ...
        const entryPrice = Math.max(0.01, parseFloat((0.0015 * Math.sqrt(currentTotalRaised)).toFixed(4)));
        currentTotalRaised += amount;
        const newPrice = Math.max(0.01, parseFloat((0.0015 * Math.sqrt(currentTotalRaised)).toFixed(4)));
        ...
        const investment = this.investmentRepo.create({
          ...
          timestamp: faker.date.between({ from: startup.createdAt, to: new Date() }),
        });
      ...
      for (let c = 0; c < commentCount; c++) {
        ...
        const text = getRandomUniqueComment(startup.category, usedTexts, commenter.username);
  ...
  private async simulateInvestment() {
    ...
    const startups = await this.startupRepo.find({ where: { status: 'active' }, take: 20 });
    ...
    this.logger.log(`🤖 Live Simulator: @${investor.username} invested $${amount} into "${startup.problem.slice(0, 30)}..." (New Price: $${newPrice})`);
  }
  ...
  private async simulateComment() {
    ...
    const existingTexts = new Set<string>((startup.comments || []).map((c: any) => c.text));
    const text = getRandomUniqueComment(startup.category, existingTexts, user.username);
    ...
    await this.commentRepo.save(comment);
    this.logger.log(`🤖 Live Simulator: @${user.username} commented on "${startup.problem.slice(0, 30)}...": "${text}"`);
  }
  ...
  private async simulateNewUser() {
    ...
    const username = isFounder ? getRandomFounderUsername() : getRandomInvestorUsername();
    ...
    const newUser = this.userRepo.create({
      ...
      role,
      bio: faker.person.bio(),
      walletBalance: role === 'investor' ? faker.number.int({ min: 5000, max: 20000 }) : 10000,
      avatarUrl: faker.image.avatar(),
      badges: isFounder ? ['Verified Builder'] : [],
    });

    await this.userRepo.save(newUser);
    this.logger.log(`🤖 Live Simulator: Created new ${role} account @${newUser.username}`);
  }

  private async simulateNewPitch() {
    const founders = await this.userRepo.find({ where: { role: 'founder' }, take: 10 });
    if (founders.length === 0) return;

    const founder = faker.helpers.arrayElement(founders);
    const category = faker.helpers.arrayElement(['AI', 'Fintech', 'Climate', 'Consumer', 'Health', 'B2B']);
    ...
    this.logger.log(`🤖 Live Simulator: Founder @${founder.username} published new pitch in ${category}!`);
  }
}
